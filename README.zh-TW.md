...
因為我只付錢訂閱 Claude Pro!(開玩笑的)

其他用量追蹤工具會讓背景服務讀取你的 cookie、直接用你的 session 呼叫各家 API,一次支援一大堆 AI 服務——但這正是這個專案想避免的信任模式。要在不碰 cookie、不自己發出認證請求的前提下安全地做到這件事,就得逐一反向工程並維護每個網站自己那支沒公開文件的用量端點,而且大部分人根本不會同時用到所有這些服務。所以這個專案就只專注在 claude.ai——把一件事做好、不用多要一分信任,而不是靠多要信任去做很多事。
